...
#------------------  -----------
# CAR T flow data: batch treatment, clustering and visualization"
# 
# author: Hugh MacMillan, Carla Jaeger"
# date: 2023-12-07
# update: 2024-7-17
#-----------------------------------
...
files_to_load <- files_to_load[str_detect(files_to_load,"df_bound")]
files_to_load

df_bound <- NULL
for (file_to_load in files_to_load) {
  df_bound_bf <- read_csv(paste0("../data/",file_to_load)) %>% 
    as.data.frame()# %>% unique()  
  df_bound <- rbind.data.frame(df_bound,df_bound_bf)
}
rm(df_bound_bf)
params_now <- read_csv(paste0("../data","/params_now_",T_subtype,".csv"))
...
if (do_shift_tings) {
  ...
  # also gotta deal w/ CCR7 ---- and variation across peak batches ------ 
...
rm(df_bound_bf,df_bound_og,df_bound_new)

plot_path_now_details_data <- paste0(plot_path_now_details,"/data")
plot_path_now_details_data
...
df_bound_new <- df_bound #read_csv(paste0(plot_path_now_details_data,"/df_bound.csv"))
...
if (do_frequency_plots) {
  ...
  df_mc_frequencies_no_ctrl %>% write_csv(paste0(plot_path_now_zoom_stack,"/df_mc_frequencies_zoom.csv"))
